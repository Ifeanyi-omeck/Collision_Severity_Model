...
prep_data <- read.csv('final_dataset_darkness_lit.csv')
prep_data <- as.matrix(prep_data)


# Fitting the first Traditional ordered Probit model using the CLM function in the ordinal package.
Op_one <- clm(factor(recoded_severity) ~ road_type +  speed_limit + weather_conditions + road_surface_conditions +
...
# Fitting the zero inflated ordered probit model with the full variables.
ziop_one_dark_lit <- iop(recoded_severity ~  road_type + speed_limit + weather_conditions + road_surface_conditions + 
...
ziop_two_darklit <- iop(recoded_severity ~ road_type + speed_limit + weather_conditions + carriageway_hazards + 
                                            skidding_and_overturning + first_point_of_impact + journey_purpose_of_driver +
                                            sex_of_driver + age_of_driver + age_of_vehicle + sex_of_casualty | road_type +
                                            speed_limit + weather_conditions + carriageway_hazards + skidding_and_overturning +
                                            first_point_of_impact + journey_purpose_of_driver + sex_of_driver + age_of_driver +
                                            age_of_vehicle + sex_of_casualty, data = prep_data, type = c('ziop'))

summary(ziop_two_darklit)




# Fitting the ZIOP model again with only statistically significant variable in the ZIOP model
ziop_third_darklit <- iop(recoded_severity ~   road_type + speed_limit + carriageway_hazards + skidding_and_overturning + sex_of_driver 
                                               + sex_of_casualty | road_type + speed_limit + carriageway_hazards + skidding_and_overturning 
                                               + sex_of_driver + sex_of_casualty, data = prep_data, type = c('ziop'))
...
final_op_darklit <- clm(factor(recoded_severity) ~  road_type + speed_limit + carriageway_hazards + skidding_and_overturning + 
                                                    sex_of_driver + sex_of_casualty, data = prep_data, link = 'probit')

summary(final_op_darklit)



# Saving the results of the Zero inflated Ordered Probit to a csv file
ziop_model <- summary(ziop_third_darklit)$coefficients
ziop_model <- as.data.frame(ziop_model)
ziop_model

# Navigate to the 'Results' folder within the current working directory
setwd("Results_Darkness_lit")
write.csv(ziop_model, 'Estimates_ZIOP_darklit.csv', row.names = FALSE)


# Saving the results of the Ordered Probit model to a csv file.
Op_final_results <-  final_op_daylight$coefficients
Op_final_results <- as.data.frame(Op_final_results)
write.csv(Op_final_results, 'Estimates_OP_darklit.csv', row.names = FALSE)
...
compute_AIC_BIC_LL(final_op_daylight, "Op_model_daylight")
...
calculate_AIC_BIC_LL_ZIOP <- function(model, n ){
  ...
  # Save the data frames to separate CSV files with the specified file_suffix
  write.csv(aic_df_ziop, file = paste0("AIC_", "Ziop_model_daylight", ".csv"), row.names = FALSE)
  write.csv(bic_df_ziop, file = paste0("BIC_", "Ziop_model_daylight", ".csv"), row.names = FALSE)
  write.csv(log_likelihood_df_ziop, file = paste0("LogLikelihood_daylight_", "Ziop_model", ".csv"), row.names = FALSE)
}


# Return values for Zero inflated Ordered Probit model for Comparison
calculate_AIC_BIC_LL_ZIOP(ziop_two_daylight, 10712)
...
ziop_marginal_result <- marginal_effect_ordered(ziop_two_daylight, prep_data, rho = 0) # A custom code for Calculating Marginal effect.
