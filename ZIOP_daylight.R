...
prep_data <- read.csv('final_dataset_day_light.csv')
...
# Fitting Ordered Probit model with Interaction terms between sex of driver and speed limit
Op_two <- clm(factor(recoded_severity) ~ road_type +  speed_limit  + weather_conditions + road_surface_conditions +
                                         special_conditions_at_site + carriageway_hazards + urban_or_rural_area + vehicle_type +
                                         skidding_and_overturning + first_point_of_impact + journey_purpose_of_driver + sex_of_driver +
                                         age_of_driver + age_of_vehicle + sex_of_casualty + age_of_casualty + car_passenger +
                                         sex_of_driver:speed_limit, data = prep_data, link = 'probit')
...
# Fitting the Ordered Probit model with interaction terms between weather conditions and road surface conditions
Op_three <- clm(factor(recoded_severity) ~ road_type +  speed_limit +  weather_conditions + road_surface_conditions +
                                         special_conditions_at_site + carriageway_hazards + urban_or_rural_area + vehicle_type +
                                         skidding_and_overturning + first_point_of_impact + journey_purpose_of_driver + sex_of_driver +
                                         age_of_driver + age_of_vehicle + sex_of_casualty + age_of_casualty + car_passenger +
                                         weather_conditions:road_surface_conditions, data = prep_data, link = 'probit')
...
ziop_one_daylight <- iop(recoded_severity ~  road_type + speed_limit + weather_conditions + road_surface_conditions + 
...
ziop_two_daylight <- iop(recoded_severity ~ road_type +  speed_limit + weather_conditions + carriageway_hazards + 
                                            vehicle_type + skidding_and_overturning + first_point_of_impact  +
                                            journey_purpose_of_driver + sex_of_driver + age_of_casualty + car_passenger |
                                            road_type + speed_limit + weather_conditions + carriageway_hazards  +
                                            vehicle_type + skidding_and_overturning + first_point_of_impact  + 
                                            journey_purpose_of_driver + sex_of_driver + age_of_casualty + car_passenger,
                                            data = prep_data, type = c('ziop'))

summary(ziop_two_daylight)


# Fit a standard ordered probit model with the same variables for comparison
final_op_daylight <- clm(factor(recoded_severity) ~ road_type + speed_limit + weather_conditions + carriageway_hazards +
                                       vehicle_type + skidding_and_overturning + first_point_of_impact + 
                                       journey_purpose_of_driver + sex_of_driver + age_of_casualty + car_passenger,
                                       data = prep_data, link = 'probit')


summary(final_op_daylight)


# Saving the results of the Zero inflated Ordered Probit to a csv file
ziop_model <- summary(ziop_two_daylight)$coefficients
ziop_model <- as.data.frame(ziop_model)
ziop_model

# Navigate to the 'Results' folder within the current working directory
setwd("Results")
write.csv(ziop_model, 'Estimates_ZIOP_daylight.csv', row.names = FALSE)
...
voung_result <- vuong_test(final_op_daylight, ziop_two_daylight, prep_data) # A custom code that calculates the Vuong test.
